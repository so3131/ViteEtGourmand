...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function calculerFrais(ville, lat, lon) {
        ...
        fetch(`index.php?page=ajax-frais-livraison&ville=${encodeURIComponent(ville)}&lat=${lat}&lon=${lon}`)
    ...
    if (inputVille && inputVille.value && inputLat && inputLat.value && inputLon && inputLon.value) {
    ...
    document.addEventListener('click', function(e) {
        if (inputAdresse && divSuggestions && !inputAdresse.contains(e.target) && !divSuggestions.contains(e.target)) {
            divSuggestions.style.display = 'none';
        }
    });
});