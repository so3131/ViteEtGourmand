...
document.getElementById('updateOrderForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const response = await fetch(`index.php?page=update-order-common&commande_id=${commandeId}`, {
            method: 'POST',
            ...
            if (data.features && data.features.length > 0) {
                data.features.forEach(feature => {
                    ...
                    item.addEventListener('click', (ev) => {
                        ...
                        const latEl = document.getElementById('lat') || document.getElementById('latitude');
                        if (latEl) latEl.value = feature.geometry.coordinates[1];

                        const lonEl = document.getElementById('lon') || document.getElementById('longitude');
...
document.querySelectorAll('input, select').forEach(el => {
    ['input', 'change'].forEach(eventType => {
        el.addEventListener(eventType, async () => {
            ...
            try {
                const commandeId = form.dataset.id;
                // Vérifie que cette route existe bien dans ton routeur PHP pour le recalcul
                const response = await fetch(`index.php?page=recalculer-prix-common&commande_id=${commandeId}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
                ...
                if (result.frais_livraison !== undefined) {
                    const fraisDisplay = document.getElementById('montant-frais') || document.getElementById('frais-livraison-display');
